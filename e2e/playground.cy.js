describe("Cypress Playground", () => {
  beforeEach(() => {
    const now = new Date(Date.UTC(2024, 6, 29));
    cy.clock(now);
    cy.visit(
  ...
  it('types in an input which "signs" a form, then asserts it is signed', () => {
    cy.get("#signature-textarea").type("Matheus");
    cy.contains("#signature", "Matheus").should("be.visible");
  });

  it("types in the signature field, checks the checkbox to see the preview, then unchecks it", () => {
    cy.get("#signature-textarea-with-checkbox").type("Matheus");
    cy.get("#signature-checkbox").check();
    cy.contains("#signature-triggered-by-check", "Matheus").should(
      "be.visible"
    );
    cy.get("#signature-checkbox").uncheck();
    cy.contains("#signature-triggered-by-check", "Matheus").should("not.exist");
  });
  ...
  it("select a type via the dropdown field and asserts on the selection", () => {
  ...
  it("select multiple fruits via the dropdown field and asserts on the selection", () => {
    ...
    cy.contains(
      "p",
      "You've selected the following fruits: apple, banana, cherry"
    ).should("be.visible");
  });
  ...
  it("selects a date and asserts the correct date has been displayed", () => {
    cy.get("#date").type("2024-11-29").blur();

    cy.contains("p", "The date you've selected is: 2024-11-29").should(
      "be.visible"
    );
  });
  ...
  it("freezes the browser clocks and asserts the frozen date is displayed", () => {
    cy.contains("p", "Current date: 2024-07-29").should("be.visible");
  });
  ...
  it("copies the code, types it, submits it, then asserts on the success message", () => {
    cy.get("#timestamp").then((element) => {
      ...
      cy.contains("Congrats! You've entered the correct code.").should(
        "be.visible"
      );
    });
  });

  it("types an incorret code and asserts on the error message", () => {
    cy.get("#code").type("12456789907");
    cy.contains("button", "Submit").click();
    cy.contains("The provided code isn't correct. Please, try again.").should(
      "be.visible"
    );
  });
